...
| `lem:wiener-phase` (algebraic core) | `WienerPhaseAlgebraCore.norm_finite_localized_phase_le`, `localized_exp_hasSum`, `norm_localized_exp_le_exp_norm`, `norm_localized_real_phase_le`, `norm_localized_real_phase_times_F_le`, `hasDerivAt_localized_exp` | **Checked for an abstract complex Banach algebra only.** Finite coefficient bounds, the localized exponential series, its quantitative norm bound, the real-time phase and phase-times-F bounds, and its complex-time derivative are proved. Concrete Wiener-algebra membership and the time-maximal inverse-Fourier kernel estimate remain outside Lean. |
| `eq:D-wiener` | — | **Analytically audited, not machine-checked.** See `WIENER_PHASE_AUDIT.md`. |
