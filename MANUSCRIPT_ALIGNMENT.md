...
| `lem:wiener-phase` (algebraic core) | `WienerPhaseAlgebraCore.norm_finite_localized_phase_le`, `localized_exp_hasSum`, `hasDerivAt_localized_exp` | **Checked for an abstract complex Banach algebra only.** Finite coefficient bounds, the localized exponential series, and its complex-time derivative are proved. The concrete Wiener algebra, quantitative real-time bound, and inverse-Fourier kernel estimate remain outside Lean. |
